...
class RDT21Sender:
    ...
    def send(self, data, dest_address):
        ...
        # Loop Stop-and-Wait com números de sequência
        while True:
            # Enviar pacote
            self._send_packet(packet)
            self.packets_sent += 1
            
            # Aguardar ACK com número de sequência correto
            response = self._wait_for_response(timeout=5.0)
            
            if response is None:
                self.logger.warning(f"Timeout aguardando ACK{self.seq_num}")
            ...
            if response.type == PacketType.ACK:
                if response.seq_num == self.seq_num:
                    ...
                    # ACK com número errado (duplicado)
...
class RDT21Receiver:
    ...
    def _send_ack(self, dest_addr, seq_num):
        """Envia ACK com número de sequência"""
